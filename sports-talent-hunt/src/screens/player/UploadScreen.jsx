...
// src/screens/player/UploadScreen.jsx
import React, { useState, useRef } from 'react';
// FIX: Import Modal and StyleSheet
...
const UploadScreen = ({ navigation }) => {
  ...
  const handleUpload = async () => {
    if (!selectedVideo || !selectedSport) {
        showMessage({ message: 'Please select a video and sport.', type: 'warning' });
        return;
    }
    ...
    try {
      ...
      showMessage({ message: 'Video uploaded successfully!', type: 'success' });
      navigation.navigate('AnalysisResult', { analysisId: result.id });

    } catch (error) {
      showMessage({ message: error.message || 'Upload failed', type: 'danger' });
    } finally {
      setUploading(false);
    }
  };
  ...
  if (uploading) {
    return <LoadingSpinner text="Uploading & Analyzing..." overlay />;
  }
  ...
  return (
    <SafeAreaWrapper>
      ...
      {/* FIX: The camera is now rendered in a Modal on top of the screen */}
      <Modal
        visible={showCamera}
        animationType="slide"
      ...
      >
        <Camera ref={cameraRef} style={StyleSheet.absoluteFill}>
          <View className="flex-1 justify-end p-6 bg-transparent">
            <View className="flex-row items-center justify-around">
              ...
              <TouchableOpacity
                ...
                className="bg-red-500 rounded-full p-5 border-4 border-white"
              >
                <Ionicons name="videocam" size={32} color="white" />
              </TouchableOpacity>
      ...
      <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="mb-6">
          ...
          {selectedVideo ? (
            ...
            <View className="flex-row space-x-3 mb-4">
              <TouchableOpacity onPress={recordVideo} className="flex-1 bg-white rounded-2xl p-6 shadow-sm items-center">
                <Ionicons name="videocam" size={32} color="#22c55e" />
                <Text className="text-gray-900 font-semibold mt-2">Record Video</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={pickVideoFromGallery} className="flex-1 bg-white rounded-2xl p-6 shadow-sm items-center">
                <Ionicons name="images" size={32} color="#f97316" />
                <Text className="text-gray-900 font-semibold mt-2">Choose from Gallery</Text>
              </TouchableOpacity>
            </View>
